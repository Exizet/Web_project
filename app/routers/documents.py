...
@router.get('/')
async def get_doc_page(db: Annotated[Session, Depends(get_db)], request: Request, get_user = Depends(get_current_user)):
    if get_user.get('is_user'):
        snt = db.scalars(select(Snt).where(Snt.id == get_user.get('snt_id'), Snt.is_active == True)).all()
        return templates.TemplateResponse('documents.html', {'request': request, 'snt': snt[0]})